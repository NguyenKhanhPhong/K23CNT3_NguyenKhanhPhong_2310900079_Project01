...
<body>
    ...
    <div class="wrap">
        <h1>Login</h1>
        <form action="{{ route('admins.nkpLoginSubmit') }}" method="POST">
            @csrf

            <div class="user-box">
                <input type="text" name="nkpTaiKhoan" id="nkpTaiKhoan" value="{{ old('nkpTaiKhoan') }}" />
                <label for="nkpTaiKhoan">Username</label>
                @error('nkpTaiKhoan')
            ...
            <div class="user-box">
                <input type="password" name="nkpMatKhau" id="nkpMatKhau" />
                <label for="nkpMatKhau">Password</label>
                @error('nkpMatKhau')
                    <div class="error-message">{{ $message }}</div>
                @enderror
            </div>
    ...
    <script>
      ...
      function toggleForgetPassword() {
        ...
        if (isChecked) {
          const username = document.getElementById('nkpTaiKhoan').value;
          const password = document.getElementById('nkpMatKhau').value;
          document.cookie = `nkpTaiKhoan=${username}; path=/; max-age=31536000`; // 1 year
          document.cookie = `nkpMatKhau=${password}; path=/; max-age=31536000`; // 1 year
        } else {
          // Remove cookies when "Remember me" is unchecked
          document.cookie = 'nkpTaiKhoan=; path=/; expires=Thu, 01 Jan 1970 00:00:00 UTC';
          document.cookie = 'nkpMatKhau=; path=/; expires=Thu, 01 Jan 1970 00:00:00 UTC';
        }
      }
      ...
      window.onload = function() {
        ...
        cookies.forEach(cookie => {
          if (cookie.trim().startsWith('nkpTaiKhoan=')) {
            username = cookie.trim().split('=')[1];
          }
          if (cookie.trim().startsWith('nkpMatKhau=')) {
            password = cookie.trim().split('=')[1];
          }
        });

        if (username && password) {
          document.getElementById('nkpTaiKhoan').value = username;
          document.getElementById('nkpMatKhau').value = password;
